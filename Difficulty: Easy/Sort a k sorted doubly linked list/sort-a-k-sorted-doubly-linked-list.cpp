...
class Solution {
  public:
    ...
    DLLNode* sortAKSortedDLL(DLLNode* head, int k) {
        ...
        // Priority queue to store node pointers instead of values
        ...
        // Push first k+1 nodes into the priority queue
        for (int i = 0; curr && i <= k; ++i) {
            pq.push(curr);
            curr = curr->next;
        }

        // Now process the list using the priority queue
        while (!pq.empty()) {
            ...
            // If there are still nodes left in the unsorted part of the list, push the next one
            if (curr) {
                pq.push(curr);
                curr = curr->next;
            }
