...
public:
    Node* copyList(Node* head) {
        ...
        Node* cur = head;
        while (cur != NULL) {
            Node* newNode = new Node(cur->data);  // Create new node
            newNode->next = cur->next;            // Set next pointer of the new node
            cur->next = newNode;                  // Insert new node after current node
        ...
        while (orig != NULL) {
            orig->next = orig->next->next;  // Restore the original list
            if (copy->next != NULL) {
                copy->next = copy->next->next;  // Extract the copied list
            }
            orig = orig->next;  // Move to the next original node
